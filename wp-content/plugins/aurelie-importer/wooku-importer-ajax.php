...
*/
    ...
    if(isset($post_data['uploaded_file_path'])) {
        ...
        if ( $handle !== FALSE ) {
            ...
            $error_messages[] = __( 'Could not open CSV file.', 'wooku-importer' );
        }

        if(sizeof($import_data) == 0) {
            $error_messages[] = __( 'No data found in CSV file.', 'wooku-importer' );
        }
...
//var_dump($new_post);var_dump($existing_product);die();
            if(strlen($new_post['post_title']) > 0 || $existing_product !== null) {

                //insert/update product
                if($existing_product !== null) {
                    $new_post_messages[] = sprintf( __( 'Updating product with ID %s.', 'wooku-importer' ), $existing_product->ID );
                ...
                if(is_wp_error($new_post_id)) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t insert product with name %s.', 'wooku-importer' ), $new_post['post_title'] );
                } elseif($new_post_id == 0) {
                    $new_post_errors[] = sprintf( __( 'Couldn\'t update product with ID %s.', 'wooku-importer' ), $new_post['ID'] );
                } else {
                    //insert successful!
                    ...
                    foreach($new_post_image_urls as $image_index => $image_url) {
                        ...
                        //$image_url = get_site_url() . "/wp-content/uploads/importer/" . str_replace(' ', '%20', trim($image_url));
                        /*
                            uploads changed by daniel to fetch images directly from other servers...
                        */
                        ...
                        if(!in_array($image_ext, $allowed_extensions)) {
                            $new_post_errors[] = sprintf( __( 'A valid file extension wasn\'t found in %s. Extension found was %s. Allowed extensions are: %s.', 'wooku-importer' ), $image_url, $image_ext, implode( ',', $allowed_extensions ) );
                            continue;
                        }
                        ...
                        if(ini_get('allow_url_fopen')) {
                            ...
                            if( ! @copy($image_url, $dest_path)) {
                                ...
                                $new_post_errors[] = sprintf( __( '%s encountered while attempting to download %s', 'wooku-importer' ), '{$http_status}', $image_url );
                            }
                            ...
                            if($http_status != 200) {
                                unlink($dest_path);
                                $new_post_errors[] = sprintf( __( 'HTTP status %s encountered while attempting to download %s', 'wooku-importer' ), '{$http_status}', $image_url );
                            }
                        } else {
                            //well, damn. no joy, as they say.
                            $error_messages[] = sprintf( __( 'Looks like %s is off and %s is not enabled. No images were imported.', 'wooku-importer' ), '<code>allow_url_fopen</code>', '<code>cURL</code>'  );
                            break;
                        }

                        //make sure we actually got the file.
                        if(!file_exists($dest_path)) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t download file %s.', 'wooku-importer' ), $image_url );
                            continue;
                        }
                    ...
                    foreach($new_post_image_paths as $image_index => $dest_path_info) {
                        ...
                        if($existing_product !== null && intval($post_data['product_image_skip_duplicates'][$key]) == 1) {
                            ...
                            if(is_array($existing_attachments) && sizeof($existing_attachments) > 0) {
                                ...
                                $new_post_messages[] = sprintf( __( 'Skipping import of duplicate image %s.', 'wooku-importer' ), $dest_path_info['source'] );
                                continue;
                            }
                        }
                        ...
                        if(!file_exists($dest_path_info['path'])) {
                            $new_post_errors[] = sprintf( __( 'Couldn\'t find local file %s.', 'wooku-importer' ), $dest_path_info['path'] );
                            continue;
                        }
                ...
                $new_post_errors[] = __( 'Skipped import of product without a name', 'wooku-importer' );
            }
