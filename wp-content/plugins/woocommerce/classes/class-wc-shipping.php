...
class WC_Shipping {
	...
	public function convert_available_methods($available_methods){
		...
		$crg = new cargonizer($cargonizer['api_key'], $cargonizer['sender_id'], $cargonizer['transport_cost_url']);
		...
		if ($shipping_postcode){
			//$service_partner = $crg->get_service_partner($shipping_postcode);
			$service_partners = $crg->get_service_partners($shipping_postcode);
		}
		...
		foreach ($available_methods as $key=>$value){
			if ($key == 'aurelie_shipping__' . md5('På Døren')){
				$shipping_method_name = "På Døren";
				$crg_data['consignments'] = array(
					"consignment" => array(
						"_attribs" => array(
							"transport_agreement" => $cargonizer['transport_agreement_bring'],
							"estimate" => "true",
						),
						...
						//dont overload the xml for cost estimate only
						/*"references" => array(
							//"consignor" => "123456",
				...
				$shipping_method_name = "Minipakke";
				$crg_data['consignments'] = array(
					"consignment" => array(
						"_attribs" => array(
							"transport_agreement" => $cargonizer['transport_agreement_bring'],
				...
				$shipping_method_name = "MyPack";

				$crg_data['consignments'] = array(
					"consignment" => array(
						"_attribs" => array(
							"transport_agreement" => $cargonizer['transport_agreement_tollpost'],
							"estimate" => "true",
						),
						"transfer_date" => date("Y-m-d\TH:i:s",strtotime("+2 hour")),
						"values" => array(
						...
						"parts" => array(
							...
							"service_partner" => array(
								"number" => "3042033",
								"customer_number" => $service_partners[0]['customer_number'],
								"name" => $service_partners[0]['name'],
								"address1" => $service_partners[0]['address1'],
								"country" => $service_partners[0]['country'],
								"postcode" => $service_partners[0]['postcode'],
								"city" => $service_partners[0]['city']
							)
						),
						"items" => $items, 
						"services" => array(
			...
			if (!isset($value->fetched) && 
				...
				if (!$error) {
					...
					$available_methods[$key]->error = $error;
				}
				else {
					$available_methods[$key]->error = $error;
				}
			...
			if ($key == 'aurelie_shipping__' . md5('MyPack')){
				if (!$service_partners){
					$available_methods[$key]->error = "Tollpost have technical problems which have affected us. Please try again in a few moments.";
				} else {
					$available_methods[$key]->service_partners = $service_partners;
				}
			}
