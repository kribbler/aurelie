...
function theretailer_scripts() {  
	...
	wp_register_script('google_map', 'http://maps.googleapis.com/maps/api/js?key=AIzaSyAK8atD3RrGX5BGwvcmO5wDEgJedI6Ab8A&sensor=false');
                                ...
                                echo '<li class="cart_list_product">';                                                
                                    ...
                                    echo '<div class="cart_list_product_title">';
                                        ...
                                        if (!$_product->get_weight()){
                                        	// this should warn Kjetil of products with no weight
											//echo "<span style='color:red'>Warning! No Weight Defined</span>";
                                        }
                                    echo '</div>';
...
/**
 * this is to remove the reviews tab
 */
...
add_filter( 'woocommerce_product_tabs', 'sb_remove_product_all_tabs', 98);
function sb_remove_product_all_tabs($tabs) {
	return NULL;
}

/**
 * remove the admin bar and redirect to / for non-admins
 * redirect_user_on_role() not used anymore.
 */
...
add_action( 'woocommerce_special_thankyou_bacs', 'my_thankyou_page' );

function my_thankyou_page(){
	$wcg_bacs = new WC_Gateway_BACS();

	$fields = apply_filters('woocommerce_bacs_fields', array(
		'account_name' 	=> __( 'Account Name', 'woocommerce' ),
		'account_number'=> __( 'Account Number for Payment Method', 'theretailer' ),
	...
	foreach ( $fields as $key=>$value ) {
	    if ( ! empty( $wcg_bacs->$key ) ) {
	    	echo '<li class="' . esc_attr( $key ) . '">' . esc_attr( $value ) . ': <strong>' . wptexturize( $wcg_bacs->$key ) . '</strong></li>';
	    }
	}
}

add_action( 'woocommerce_special_info_email', 'special_info_email' );

function special_info_email($order){
		
	echo '<li style="">' . __( 'Order:', 'woocommerce' ) . '&nbsp;<b>' . $order->get_order_number() . '</b></li>';
	echo '<li style="">' . __( 'Date:', 'woocommerce' ) . '&nbsp;<b>' . date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ) . '</b></li>';
	echo '<li style="">' . __( 'Total:', 'woocommerce' ) . '&nbsp;<b>' . $order->get_formatted_order_total() . '</b></li>';
	if ( $order->payment_method_title ) :
		echo '<li style="">' . __( 'Payment method:', 'woocommerce' ) . '&nbsp;<b>' . $order->payment_method_title . '</b>';
                echo ($order->payment_method_title == 'Kredittkort') ? " (betalt)" : "";
                echo '</li>';
		...
		//echo '<li style="">' . __( 'Package Number:', 'theretailer' ) . '&nbsp;<b><a href="http://sporing.posten.no/sporing.html?q='.$package_number.'&lang=no" target="_blank">' . $package_number . '</b></li>';
	endif;
	
}

add_action( 'woocommerce_special_info_admin_email', 'special_info_admin_email' );

function special_info_admin_email($order){
...
__("hash before order number", "theretailer");

/**
 * this bellow creates the action 'woocommerce_checkout_order_levering' 
 * that is used to display the Levering box in checkout page
 */
add_action(  'woocommerce_checkout_order_levering', 'checkout_cargonizer' );

function checkout_cargonizer(){
	global $woocommerce;
	$available_methods = $woocommerce->shipping->get_available_shipping_methods();
	woocommerce_get_template( 'checkout/form-order-levering.php', array( 'available_methods' => $available_methods ) );
}

function check_frontend_for_bring_notification_service(){
...
/**
 * Google Maps to be displayed on checkout under Minipakke and MyPack
 * where address is defined in Aurelie Shipping plugin
 */
add_action( 'show_google_map', 'google_map' );

function google_map($arg){?>
	<div id="choose_delivery_location">
		<?php $already_checked = false;?>
		<?php foreach ($arg['service_partners'] as $location){?>
			<?php 
			
			$address = $location['address1'] . ", " . $location['postcode'] . " " . $location['city'];
			$opening_hours = "";
			foreach ($location['opening-hours'] as $key=>$value){
				$opening_hours .= "<li>" . __( $key, 'theretailer' ) . ": " . $value . "</li>";	
			}
			?>
			<?php 
			if (!$already_checked){
				...
				$pickup_customer_number = $location['customer_number'];
				$pickup_name = $location['name'];
				$pickup_address1 = $location['address1'];
				$pickup_country = $location['country'];
				$pickup_postcode = $location['postcode'];
			...
			<p class="checkout_p">
				<input 	type="radio" 
						...
						onchange="change_delivery_location(
							'<?php echo $location['customer_number']?>', 
				...
				/>
				<?php echo $location['address1']?>
				<span id="fire_map_<?php echo $arg['method']?>">
					<a 	href="javascript:void(0);" 
						onclick="initialize_<?php echo $arg['method']?>(
							'<?php echo $address?>', 
							'<?php echo $lat_lng?>',
							'<?php echo $location['name']?>',
						...
						<?php _e( 'View Delivery Location', 'theretailer' )?>
					</a>
				<span>
			</p>
		<?php }?>
		<div id="gm_h_<?php echo $arg['method']?>" class="gm_h">
			<div id="gm_o_h_<?php echo $arg['method']?>" class="opening_hours">
				<br /><p class="checkout_p"><b><span id="location_name"><?php echo $location['name']?></span></b></p>
				<ul id="opening_hours"></ul>
				<?php /*
				<ul>
					<?php foreach ($location['opening-hours'] as $key=>$value){?>
					<li><?php _e( $key, 'theretailer' )?>: <?php echo $value?></li>	
					<?php } ?>
				</ul>
				*/?>
			</div>
			<div id="googleMap_<?php echo $arg['method']?>" class="googleMap"></div>
		</div>
		<input type="hidden" id="pickup_customer_number" name="pickup_customer_number" value="<?php echo $pickup_customer_number?>" />
		<input type="hidden" id="pickup_name" name="pickup_name" value="<?php echo $pickup_name?>" />
		<input type="hidden" id="pickup_address1" name="pickup_address1" value="<?php echo $pickup_address1?>" />
		<input type="hidden" id="pickup_country" name="pickup_country" value="<?php echo $pickup_country?>" />
		<input type="hidden" id="pickup_postcode" name="pickup_postcode" value="<?php echo $pickup_postcode?>" />
		<input type="hidden" id="pickup_city" name="pickup_city" value="<?php echo $pickup_city?>" />
	</div>
	
	<script type="text/javascript">
	function change_delivery_location(customer_number, location_name, address1, country, postcode, city){
		document.getElementById("pickup_customer_number").value = customer_number;
		document.getElementById("pickup_name").value = location_name;
		document.getElementById("pickup_address1").value = address1;
		document.getElementById("pickup_country").value = country;
		document.getElementById("pickup_postcode").value = postcode;
		document.getElementById("pickup_city").value = city;
	}
	
	function initialize_<?php echo $arg['method']?>(address, lat_lng, location_name, opening_hours, customer_number, address1, country, postcode, city) {
		var googleMap_holder = document.getElementById("gm_h_<?php echo $arg['method']?>");
		googleMap_holder.style.display = "block";

		var myCenter=new google.maps.LatLng(<?php echo geocode_googlemap_address($address);?>);
		var mapProp = {
				center:myCenter,
				zoom:15,
				mapTypeId:google.maps.MapTypeId.ROADMAP
			};
		var map=new google.maps.Map(document.getElementById("googleMap_<?php echo $arg['method']?>"), mapProp);
		...
		document.getElementById("opening_hours").innerHTML = opening_hours;

		change_delivery_location(customer_number, location_name, location_name, country, postcode, city);
/*
		document.getElementById("pickup_customer_number").value = customer_number;
		document.getElementById("pickup_name").value = location_name;
		document.getElementById("pickup_address1").value = address1;
...
function google_map_old($arg){

	$address = $arg['service_partners'][0]['address1'] . ", " . $arg['service_partners'][0]['postcode'] . " " . $arg['service_partners'][0]['city'];
	
	//$address = get_pickup_address();
	?>
	
	<script>
	function initialize_<?php echo $arg['method']?>(address) {
		var googleMap_holder = document.getElementById("gm_h_<?php echo $arg['method']?>");
		googleMap_holder.style.display = "block";
	...
	<div id="choose_delivery_location">
		<?php foreach ($arg['service_partners'] as $location){?>
			<p class="checkout_p"><input type="radio" name="delivery_locations">
				<?php echo $location['address1']?>
				<span id="fire_map_<?php echo $arg['method']?>"><a href="javascript:void(0);" onclick="initialize_<?php echo $arg['method']?>('<?php echo $location['address1']?>');"><?php _e( 'View Delivery Location', 'theretailer' )?></a><span>
				<div id="gm_h_<?php echo $arg['method']?>" class="gm_h">
					<div id="gm_o_h_<?php echo $arg['method']?>" class="opening_hours">
						<br /><p class="checkout_p"><b><?php echo $location['name']?></b></p>
						<ul>
							<?php foreach ($location['opening-hours'] as $key=>$value){?>
							<li><?php _e( $key, 'theretailer' )?>: <?php echo $value?></li>	
							<?php } ?>
...
function get_pickup_address(){
	$option = get_option( 'woocommerce_aurelie_shipping_settings' );
	$address = $option['pickup_address'] . ", " . $option['pickup_postcode'] . " " . $option['pickup_city'];
	return $address;
}
...
function switch_via($v){
	$label = __( 'Shipping', 'theretailer' );
	$x = explode("&nbsp;<small>via ", $v['value']);
	if (isset($x[1])){
		$new = array();
		$new['label'] = $label . "&nbsp;(<small>" . __($x[1], 'theretailer') . "):";
		$new['value'] = $x[0];
		$new['remove_p'] = false;
	} else {
		$new = array();
...
function load_touch_punch_js() {
	...
	wp_register_script( 'woo-jquery-touch-punch', get_template_directory_uri() . "/js/jquery.ui.touch-punch.min.js", array('jquery'), $version, true );
