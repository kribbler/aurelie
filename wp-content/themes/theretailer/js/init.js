...
var gbtr_order_levering_content_global_var = 'close';
...
jQuery(document).ready(function($) {
	...
	$('.gbtr_shipping_address_header').click(function() {
		...
		gbtr_order_levering_content_global_var = 'close';
		$(this).toggleClass('gbtr_checkout_header_nonactive');
	...
	/**
	 * code added by daniel
	 */
	
	$('.gbtr_order_levering_header').live('click', 	function() {
		$(this).toggleClass("gbtr_checkout_header_nonactive");
		//gbtr_order_review_content_global_var = 'close';
		
		$('.gbtr_order_levering_content').slideToggle(200, function() {
			// Animation complete.
			gbtr_order_levering_content_global_var = "open";
	...
	$('.button_billing_address_continue').click(function () {		
		...
		if( $(".gbtr_order_notes_content").length > 0 ) {
			...
			$('.gbtr_order_levering_header').toggleClass("gbtr_checkout_header_nonactive");
			$('.gbtr_order_levering_content').slideDown(500, function() {
				// Animation complete.
			});
	...
	/**
	 * bellow is to get autocheck SMS/email radio
	 * by daniel
	 */
	var my_shipping_method = $('input[name=shipping_method]');
	var selected_id = my_shipping_method.filter(':checked').val();
	
	$('.input_retrieve').live('focusin', function(){
		var my_shipping_method = $('input[name=shipping_method]');
		var selected_id = my_shipping_method.filter(':checked').val();

		console.log('#retrieve_method_sms_number_' + selected_id);
		$('#retrieve_method_sms_number_' + selected_id).focus(function(){
			$('#retrieve_method_sms_' + selected_id).attr('checked', true);
		});
		$('#retrieve_method_sms_number_' + selected_id).live('focusin', (function(){
			$('#retrieve_method_sms_' + selected_id).attr('checked', true);
			
		}));
		$('#retrieve_method_email_value_' + selected_id).focus(function(){
			$('#retrieve_method_email_' + selected_id).attr('checked', true);
		});
		$('#retrieve_method_email_value_' + selected_id).live('focusin', (function(){
			$('#retrieve_method_email_' + selected_id).attr('checked', true);
		}));
		
	});
	
	$('.button_order_levering_continue').live('click', function () {
		var my_shipping_method = $('input[name=shipping_method]');
		var selected_id = my_shipping_method.filter(':checked').val();
		
		/**
		 * bellow is to get the retrieval method (SMS/email)
		 * by daniel
		 */
		var my_retrieve_method = $('input[name=retrieve_method_' + selected_id + ']');
		var selected_retrieve = my_retrieve_method.filter(':checked').val();
		
		//console.log(selected_retrieve);
		
		if (!$('#retrieve_method_sms_number_' + selected_id).val() && !$('#retrieve_method_email_value_' + selected_id).val()){
			$('.choose_retrieve').css('color', 'red');
		} else {
			$('.choose_retrieve').css('color', '#555');
		}
		
		if (selected_retrieve == 'retrieve_sms_' + selected_id){
			$('#retrieve_method_email_value_' + selected_id).css('border', '1px solid #cccccc');
			if (!$('#retrieve_method_sms_number_' + selected_id).val()){
				$('#retrieve_method_sms_number_' + selected_id).css('border', '1px solid red');
				
				return false;
			} else {
				$('#retrieve_method_sms_number_' + selected_id).css('border', '1px solid #cccccc');
			}
		}
		else if (selected_retrieve == 'retrieve_email_' + selected_id){
			$('#retrieve_method_sms_number_' + selected_id).css('border', '1px solid #cccccc');
			if (!$('#retrieve_method_email_value_' + selected_id).val()){
				//console.log('no email');
				$('#retrieve_method_email_value_' + selected_id).css('border', '1px solid red');
				return false;
			} else {
				if (!valid_email($('#retrieve_method_email_value_' + selected_id).val())){
		...
		//finished 'bellow is to get the retrieval method (SMS/email)'
...
jQuery(document).ajaxStop(function() {
	...
	if (gbtr_order_levering_content_global_var === "open") {
		
		jQuery('.gbtr_order_levering_content').show();
		jQuery(".gbtr_order_levering_header").removeClass("gbtr_checkout_header_nonactive");
		
	} else {
		
		jQuery(".gbtr_order_levering_header").addClass("gbtr_checkout_header_nonactive");
